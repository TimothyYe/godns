...
var (
	// DreamhostURL the API address for he.net
...
func (handler *Handler) UpdateIP(hostname, currentIP, lastIP string) {
	...
	updateDNS(lastIP, currentIP, hostname, "remove")
	updateDNS(lastIP, currentIP, hostname, "add")

}

// updateDNS can add or remove DNS records.
func (handler *Handler) updateDNS(dns, ip, hostname, action string) string {
	// Generates UUID
	uid, _ := uuid.NewRandom()
	values := url.Values{}
	values.Add("record", hostname)
	values.Add("key", handler.Configuration.LoginToken)
	values.Add("type", handler.Configuration.IPType)
